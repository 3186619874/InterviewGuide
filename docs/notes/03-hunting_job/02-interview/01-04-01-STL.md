...
C++11正是通过引入右值引用来优化性能，具体来说是通过移动语义来避免无谓拷贝的问题，通过move语义来将临时生成的左值中的资源无代价的转移到另外一个对象中去（我认为这句描述地不够恰到，解释见后），通过完美转发来解决不能按照参数实际类型来转发的问题（同时，完美转发获得的一个好处是可以实现移动语义）。 （move本身不具备任何移动的效果，move底层使用了remove_reference_t将原来的引用修饰符删除，然后再强转返回。不管原来是左值引用还是右值引用，move以后都变为右值引用）
